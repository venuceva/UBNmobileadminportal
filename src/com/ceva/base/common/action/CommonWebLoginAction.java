...
//System.out.println("location ::: "+responseDTO.getData().get("Location"));
					...
					if (userStatus.equals("A")) {
						...
						CommonLoginDAO otpLoginDAO = null;
		       responseDTO = prepaidLoginDAO.sendOtp(userBean.getUserid());
						
						String userid=(userBean.getUserid());
						
				
						logger.debug("venuroute");
						redirectPage = "WEB-INF/jsp/otpPage.jsp";
	...
	public String sendOtp() {
		...
		try {
			...
			logger.debug("Got exception in otp [" + e.getMessage()
					+ "]");
			redirectPage = "WEB-INF/jsp/otpPage.jsp";
			result = "otpfail";
		} finally {
	...
	public String webLogOut() {
		...
		/* redirectPage = "WEB-INF/jsp/commonLogin.jsp"; */
		/*
		 * System.out.println("rajeshnew"); session =
		 * ServletActionContext.getRequest().getSession(false);
		 * session.getAttribute("MENU_DATA");
		 */
