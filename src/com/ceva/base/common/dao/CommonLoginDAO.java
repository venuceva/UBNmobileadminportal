...
import com.ceva.unionbank.db.ConnectionHelper;
...
public class CommonLoginDAO {
	...
	public ResponseDTO sendOtp(String userid) {
		...
		responseDTO=new ResponseDTO();
		 ...
		 query ="select mobile_NO   from USER_LOGIN_CREDENTIALS ACM , user_information ACI "
					+ "where ACI.COMMON_ID=ACI.COMMON_ID and upper(ACM.LOGIN_USER_ID)=upper(?)";
					
			logger.info("Init product Query "+query);
			logger.info("otpselecthem ["+userid+"]");
			logger.info("userid ["+userid+"]");

			pstmt = connection.prepareStatement(query);
			logger.info("otpselect ["+userid+"]");
			pstmt.setString(1, userid);
			logger.info("otpselectone ["+userid+"]");
			rs = pstmt.executeQuery();
	     	
			logger.info("otpselecttwo ["+userid+"]");
			if (rs.next()) {
			
				 phoneno = rs.getString(1);
			
			}
			logger.info("otpselecttthre ["+userid+"]");
			if(phoneno !=null){


				/*
				 * String otpQry=
				 * "INSERT INTO OTP_DATA_TABL( REF_NO,USER_ID,MOBILE_NO,OTP,CHANNEL,"+
				 * "TRANS_DTTM,OTP_STATUS , TRANS_TYPE) "+
				 * " values(ACT_OTP_SEQ.NEXTVAL,?,?,?,?,sysdate,?,?)" ;
				 */
				

				String otpQry= "INSERT INTO OTP_DATA_TABL( REF_NO,USER_ID,MOBILE_NO,OTP,CHANNEL,"+
						"TRANS_DTTM,OTP_STATUS , TRANS_TYPE) "+
						" values('100',?,?,?,?,sysdate,?,?)" ;
				
				logger.info("otpQry ["+otpQry+"]");
				
				String otp = CommonOtploginDAO.createRandomNumber(6);
				pstmt = connection.prepareStatement(otpQry);
				pstmt.setString(1, userid);
				pstmt.setString(2, phoneno);
				pstmt.setString(3, otp);
				pstmt.setString(4, "MOBILE");
				pstmt.setString(5, "O");
				pstmt.setString(6, "LOGINOTP" );
				...
				if (i == 1 ) {
					...
					smsRequest.put("otp", otp );
					logger.info("sendsmslogindao ");
					JSONObject smsresp =  CommonOtploginDAO.sendSms(phoneno, CommonOtploginDAO.TOKENLOGINAUTH , smsRequest , "MOBILE");
					logger.info("smsresp ["+smsresp+"]");
					responseJson.put("smsrespcode", smsresp.has("respcode")?smsresp.getString("respcode"):"");
					responseJson.put("smsrespdesc", smsresp.has("respdesc")?smsresp.getString("respdesc"):"");
					//responseDTO.addMessages("success");
					int resCnt = callableStatement.getInt(1);
					if (resCnt == 1) {
						responseDTO
								.addMessages("OTP Sent  Successfully for registered mobile number. ");
					} else {
						responseDTO
								.addError("There is an issue in OTP ");
					}
				} else {

					responseJson.put("respcode", "01");
					responseJson.put("respdesc", "Fail" );

					logger.debug("insertion failed due to some error ");
